    ...
    def upload_cv(self, request, pk=None):
        ...
        def process_cv():
            try:
                ...
                with transaction.atomic():
                    # Clear existing CV-extracted skills
...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        if user.role == 'EMPLOYEE':
            # Employee dashboard - show their own stats
            employee = getattr(user, 'employee_profile', None)
            if not employee:
                return Response({
                    'active_tasks': 0,
            ...
            data = {
                ...
                'unassigned_tasks': 0,  # Not relevant for employees
                'employee_capacity': round(capacity, 1),
                'skills_coverage': 0  # Not relevant for employees
            }
        else:
            # PM dashboard - show team stats
            ...
            # Skills coverage (percentage of tasks with matching employee skills)
