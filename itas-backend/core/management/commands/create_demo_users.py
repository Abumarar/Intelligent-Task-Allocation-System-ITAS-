...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            pm_user.save()
            self.stdout.write(
                self.style.SUCCESS(f"Created PM user: {pm_user.username}")
            )
        else:
            ...
            pm_user.save()
            self.stdout.write(
                self.style.WARNING(f"PM user already exists: {pm_user.username}")
            )
        ...
        if created:
            ...
            emp_user.save()
            self.stdout.write(
                self.style.SUCCESS(f"Created Employee user: {emp_user.username}")
            )
        else:
            # Update password in case it was changed
            emp_user.set_password("emp123")
            emp_user.save()
            self.stdout.write(
                self.style.WARNING(f"Employee user already exists: {emp_user.username}")
            )
        ...
        if emp_created:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Created employee profile with skills")
            )
