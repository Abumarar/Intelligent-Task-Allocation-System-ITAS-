import pytest
from core.models import Employee, Project, Task, User
...
def pm_user():
    user = User.objects.create_user(
        username="pm_user", password="password123", role="PM"
    )
    return user
...
def employee_user():
    user = User.objects.create_user(
        username="emp_user", password="password123", role="EMPLOYEE"
    )
    return user
...
def employee_profile(employee_user, pm_user):
    return Employee.objects.create(
        user=employee_user, manager=pm_user, title="Software Engineer"
    )
...
def sample_task(pm_user, sample_project):
    return Task.objects.create(
        title="Test Task", created_by=pm_user, project=sample_project, priority="HIGH"
    )
