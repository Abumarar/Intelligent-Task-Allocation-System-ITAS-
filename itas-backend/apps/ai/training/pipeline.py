...
DATA_PATH = os.path.join(
    ...
    "../../../ai_training/dataset/task_assignment_training_dataset.csv",
)
...
def load_and_prepare_data():
    ...
    labels = [0, 1, 2, 3, 4]
    df["relevance"] = pd.cut(df["performance_score"], bins=bins, labels=labels).astype(
        int
    )
...
def train_ranker():
    ...
    db_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../../mlflow.db")
    )
    ...
    with mlflow.start_run():
        ...
        print(
            f"Model successfully trained and saved to {model_path} and registered in MLflow."
        )
